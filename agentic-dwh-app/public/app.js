...
const activityFeed = $("activityFeed"), emptyState = $("emptyState"), apiBadge = $("apiBadge");
...
let agentCards = {}, globalTimer = null, globalStart = null, finalReport = "", currentRunId = null;
let auditCount = 0, govItems = 0;

/* ── health ── */
(async () => {
  try {
    const d = await (await fetch("/api/health")).json();
    apiBadge.textContent = d.hasApiKey ? "✓ API key set — ready" : "⚠ No API key — add to .env";
    apiBadge.className = "api-badge landing-api " + (d.hasApiKey ? "ok" : "err");
  } catch { apiBadge.textContent = "✗ Server unreachable"; apiBadge.className = "api-badge landing-api err"; }
})();
...
async function fetchSources() {
  try {
    ...
    const byCat = {}; d.status.forEach((s) => (byCat[s.category] = byCat[s.category] || []).push(s));
    const extOf = {}; d.subjectAreas.forEach((a) => (extOf[a.object] = a.ext));
    for (const cat of Object.keys(d.sourceClasses)) {
      const el = $("areas-" + cat); if (!el) continue;
      el.innerHTML = (byCat[cat] || []).map((s) => `
        <div class="area-chip ${s.source}">
          <a class="area-dl" href="/samples/${cat}/${s.object}.${extOf[s.object]}" download title="Download sample">${s.label}</a>
          <span class="area-meta">${s.rows.toLocaleString()} · ${s.source}</span>
        </div>`).join("");
    }
    const ready = d.status.every((s) => s.source !== "missing");
    btnLaunch.disabled = !ready;
    const up = d.status.filter((s) => s.source === "uploaded").length;
    landingStatus.textContent = ready
      ? `${d.status.length} subject areas ready${up ? ` (${up} uploaded)` : " (sample data)"}.`
      : "Some sources missing — upload files or use sample data.";
  } catch { landingStatus.textContent = "Could not load source catalogue."; }
}
...
function wireUploads() {
  document.querySelectorAll(".upload-card").forEach((card) => {
    ...
    const drop = card.querySelector(".uc-drop");
    input.addEventListener("change", () => handleFiles(category, input.files));
    ...
    drop.addEventListener("drop", (ev) => { ev.preventDefault(); handleFiles(category, ev.dataTransfer.files); });
  });
}
async function handleFiles(category, fileList) {
  const files = [];
  for (const f of fileList) files.push({ name: f.name, content: await f.text() });
  if (!files.length) return;
  landingStatus.textContent = `Uploading ${files.length} file(s) to ${category}…`;
  try {
    await fetch("/api/upload", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ category, files }) });
    await fetchSources();
  } catch { landingStatus.textContent = "Upload failed."; }
}

btnLoadSamples.addEventListener("click", async () => {
  landingStatus.textContent = "Resetting to sample data…";
  await fetch("/api/reset-sources", { method: "POST" }).catch(() => {});
  await fetchSources();
});
...
function resetRun() {
  ...
  agentCards = {}; finalReport = ""; currentRunId = null; auditCount = 0; govItems = 0;
...
btnKill.addEventListener("click", killPipeline);
...
async function runPipeline(goalArg) {
  ...
  try {
    const resp = await fetch("/api/orchestrate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ goal }) });
    if (!resp.ok) { const e = await resp.json().catch(() => ({ error: resp.statusText })); return showError(e.error || resp.statusText); }
    const reader = resp.body.getReader(); const dec = new TextDecoder(); let buf = "";
    while (true) {
      const { value, done } = await reader.read(); if (done) break;
      buf += dec.decode(value, { stream: true }); const lines = buf.split("\n"); buf = lines.pop();
      for (const ln of lines) { if (ln.startsWith("data: ")) { try { handleEvent(JSON.parse(ln.slice(6))); } catch {} } }
    }
...
async function killPipeline() {
  ...
  try { await fetch("/api/kill", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ runId: currentRunId }) }); } catch {}
}
...
function handleEvent(ev) {
  audit(ev);
  switch (ev.type) {
    case "start": currentRunId = ev.runId; btnKill.style.display = ""; btnKill.disabled = false; btnKill.textContent = "⛔ Kill"; break;
...
function buildSummary(raw) {
  ...
  const md = t.match(/(\d+)\s+models/i); if (md) pills.push(pill(`${md[1]} models`, "info"));
  const rw = t.match(/([\d,]+)\s+rows/i); if (rw) pills.push(pill(`${rw[1]} rows`, "info"));
  if (/DRIFT DETECTED/i.test(t)) pills.push(pill("schema drift", "warn"));
...
async function decide(card, decision) {
  ...
  try { await fetch("/api/decision", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ runId: currentRunId, decision }) }); } catch {}
}
...
function renderGovAlert(ev) {
  ...
  const div = document.createElement("div"); div.className = "gov-alert " + (ev.level === "kill" ? "kill" : ev.level === "warn" ? "drift" : "drift");
  div.innerHTML = `<span class="ga-tag">${ev.level === "kill" ? "⛔ KILL" : "⚠ " + (ev.level || "DRIFT").toUpperCase()}</span> <span>${esc(ev.message || "")}</span><span class="ga-ts">${tm(ev.ts)}</span>`;
...
function audit(ev) {
  ...
  const detail = ev.type === "tool_call" ? `${ev.tool}(${JSON.stringify(ev.input || {}).slice(0, 80)})`
    : ev.type === "tool_result" ? `${ev.tool} → ${String(ev.result || "").slice(0, 90)}`
    ...
    : ev.type === "governance" ? ev.message : ev.message || (ev.result ? String(ev.result).slice(0, 90) : "");
  if (!detail) return;
...
function renderPowerBI(ev) {
  ...
  powerbiBar.innerHTML = `
    ...
    <div class="pbi-links">
      ${ev.xlsx ? `<a class="pbi-dl primary" href="${ev.xlsx}" download>⬇ Excel dataset (.xlsx)</a>` : ""}
      <a class="pbi-dl" href="${ev.pbids}" download>⬇ Power BI connector (.pbids)</a>
...
window.downloadReport = () => { if (!finalReport) return; const b = new Blob([finalReport], { type: "text/markdown" }); const a = document.createElement("a"); a.href = URL.createObjectURL(b); a.download = `gopher-migration-report-${new Date().toISOString().slice(0, 10)}.md`; a.click(); URL.revokeObjectURL(a.href); };
