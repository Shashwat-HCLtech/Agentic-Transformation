...
window.DWH = (function () {
  ...
  class Warehouse {
    ...
    buildStaging() {
      ...
      const dedupe = (rows, pk) => { const s = new Set(); return rows.filter((r) => (s.has(r[pk]) ? false : s.add(r[pk]))); };
      out("stg_group", dedupe(ensure("member_group"), "group_id").map((g) => ({ group_id: g.group_id, group_name: g.group_name, line_of_business: g.line_of_business, region: g.region })));
      out("stg_plan", dedupe(ensure("benefit"), "plan_id").map((p) => ({ plan_id: p.plan_id, plan_name: p.plan_name, formulary_tier: Number(p.formulary_tier), tier_copay: Number(p.tier_copay), line_of_business: p.line_of_business })));
      out("stg_drug", dedupe(ensure("drug_master"), "ndc").map((d) => ({ ndc: d.ndc, drug_name: d.drug_name, drug_class: d.drug_class, is_generic: d.is_generic === "Y", manufacturer: d.manufacturer, awp_unit_cost: Number(d.awp_unit_cost) })));
      out("stg_member", dedupe(ensure("eligibility"), "member_id").map((m) => ({ member_id: m.member_id, full_name: m.full_name, group_id: m.group_id, plan_id: m.plan_id, line_of_business: m.line_of_business, is_active: m.eligibility_status === "Active" })));
      out("stg_claims", ensure("pharmacy_claims").map((c) => ({ claim_id: c.claim_id, member_id: c.member_id, ndc: c.ndc, pharmacy_id: c.pharmacy_id, fill_date: c.fill_date, line_of_business: c.line_of_business, formulary_tier: Number(c.formulary_tier), quantity_dispensed: Number(c.quantity_dispensed), days_supply: Number(c.days_supply), ingredient_cost: Number(c.ingredient_cost), total_paid_amount: Number(c.total_paid_amount), is_generic: c.is_generic === "Y", drug_class: c.drug_class, claim_status: c.claim_status })));
      out("stg_rejected_claims", ensure("rejected_claims").map((r) => ({ reject_id: r.reject_id, member_id: r.member_id, ndc: r.ndc, fill_date: r.fill_date, line_of_business: r.line_of_business, reject_code: String(r.reject_code), reject_reason: r.reject_reason })));
      out("stg_rebate", ensure("rebate").map((r) => ({ rebate_id: r.rebate_id, ndc: r.ndc, manufacturer: r.manufacturer, quarter: r.quarter, rebate_pct: Number(r.rebate_pct) })));
      return built;
    }
    buildMarts() {
